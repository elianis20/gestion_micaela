...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try{
        ...
        data.forEach((element) => {
            const {id, name, lastname, url, description, birthdate, color } = element;
        ...
        >
            ...
            />
            <span class="mt-3 text-lg text-stone-500">Edad: ${calculateAge(birthdate)} años</span>
            </figure>
            ...
            <div></div>
            <div>
                <button class="deleteBtn bg-red-400 hover:text-stone-100 hover:bg-red-600 px-5 py-2 rounded">Eliminar</button>
            </div>
            </section>
        </div>`;
...
function calculateAge(birthdate) {
    ...
    const birth = new Date(partes[2], partes[1] - 1, partes[0]);
